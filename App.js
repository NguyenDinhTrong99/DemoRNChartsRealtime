...
class StockChartScreen extends React.Component {
    ...
    setInterval(() => {
      if (!this.isScroll) {
        ...
        this.setState({
          ...this.state,
          ...this.zoom,
          today: moment(this.state.today, DATE_FORMAT).add(1, 'days').format(DATE_FORMAT)
        })
        this.getData()
      }
    }, 1000)
  ...
  getData() {
    let today = moment(this.state.today, DATE_FORMAT)
    ...
    this.mockLoadData(start, today).then((data) => {
      this.zoom = this.zoom ?? { scaleX: 1.5, scaleY: 1, xValue: this.getIndexOfDay(today) - 5, yValue: 0, axisDependency: 'RIGHT' }
      this.setState({
    ...
    if (nativeEvent.action == 'chartTranslated') {
      ...
      if (!this.isLoading) {
        ...
        if (this.xMin > left - distanceToLoadMore || right + distanceToLoadMore > this.xMax) {
          ...
          let toIndex = Math.min(centerX + pageSize, moment(this.state.today).diff(era, 'days'));
          ...
          this.isScroll = !(this.getIndexOfDay(to) < right)
